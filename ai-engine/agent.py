...
async def fetch_tenant_config(tenant_id: str):
    async with await psycopg.AsyncConnection.connect(DB_URI) as conn:
        async with conn.cursor() as cur:
            ...
            return {"prompt": "You are a helpful assistant.", "hitl_mode": "SAFEGUARDS_ONLY", "is_active": True}
...
async def run_bot(thread_id: str, message: str, tenant_id: str, resume_data: dict = None) -> dict:
    ...
    if not tenant_data["is_active"]:
        return {"status": "DISABLED", "reply": "Our AI automated assistant is currently offline."}
        
    model_name = "gpt-4o-mini"
    llm = ChatOpenAI(model=model_name, temperature=0.2)
    
    # Connects to your unchanged mcp_server.py
    ...
    try:
        ...
        async def call_model(state: MessagesState, config: RunnableConfig):
            ...
            # --- PERFORMANCE UPGRADE: TOKEN TRIMMING ---
            ...
            messages = [SystemMessage(content=tenant_data["prompt"])] + trimmed_history
